...
VALUES (1120, 'SignaColor 120-A4 weiß', 1, 10, 10000, 5000),
...
VALUES (28447, 'Sauer & Esser', 'Danziger Str. 33', '48683', 'Ahaus', '02561/77-0', 0.14),
...
VALUES (14887, 1120, 47544, 26.80, '2021-04-23'),
...
ORDER BY lieferantenName
LIMIT 13;

SELECT artikelNr, artikelBezeichnung, istBestand
FROM artikel
WHERE artikelBezeichnung LIKE '%A5%';

SELECT lieferantenName, strasse
FROM lieferant
WHERE strasse LIKE '_a%'
   OR strasse LIKE '_e%';

SELECT artikelNr, artikelBezeichnung, gruppenNr, istBestand, meldeBestand
FROM artikel
WHERE (gruppenNr = 10 OR gruppenNr = 30)
  AND istBestand < meldeBestand;
...
FROM artikel
WHERE ROUND(ROUND(meldeBestand * 1.25) - istBestand) > 0;

SELECT AVG(rabattSatz)
FROM lieferant;
...
WHERE a.artikelBezeichnung LIKE '%A4 weiß'
  AND b.angebotspreis < (SELECT MIN(angebotsPreis)
                         FROM bestellung
...
FROM artikel a
         ...
         JOIN lieferant l ON l.lieferantenNr = b.lieferantenNr
WHERE a.istBestand < a.meldeBestand
  AND a.artikelBezeichnung LIKE '%DIN-A4 weiß'
GROUP BY a.artikelNr
...
SELECT b.bestellNr,
       a.artikelBezeichnung,
       l.lieferantenName,
       MONTH(b.angebotsDatum) AS monat,
       YEAR(b.angebotsDatum)  AS jahr
FROM bestellung b
...
FROM bestellung b
         JOIN lieferant l ON l.lieferantenNr = b.lieferantenNr
WHERE MONTH(b.angebotsDatum) BETWEEN 1 AND 3
ORDER BY l.lieferantenName;

INSERT INTO lieferant (lieferantenNr, lieferantenName, strasse, plz, ort)
VALUES (50000, 'FLS GmbH', 'Brunhildenstr 142', '65189', 'Wiesbaden'),
       (50001, 'FES OHG', 'Balthasar-Neumann-Straße 1', '65189', 'Wiesbaden');

UPDATE lieferant
SET telefon = '0611/315-100'
WHERE lieferantenNr = 50000;

SELECT l.lieferantenNr, l.lieferantenName, l.strasse, l.plz, l.ort, l.telefon, l.rabattSatz
FROM lieferant l
WHERE l.rabattSatz IS NULL;

DELETE
FROM lieferant
WHERE lieferantenName IN ('FLS GmbH', 'FES OHG');

INSERT INTO bestellung (bestellNr, artikelNr, lieferantenNr, angebotsPreis, angebotsDatum)
VALUES (10000, 4013, 52441, 19.5, '2021-11-01');

INSERT INTO artikel (artikelNr, artikelBezeichnung, steuerSchluessel, gruppenNr, meldeBestand, istBestand)
VALUES (10000, 'FLS Papier gelb', 1, 10, 1000, 2000),
       (10001, 'FLS Papier rot', 1, 10, 1000, 1500);

SELECT a.artikelNr, a.artikelBezeichnung
FROM artikel a
...
WHERE a.artikelNr IS NULL
  AND a.artikelBezeichnung IS NULL;

SELECT a.gruppenNr, w.gruppenName, a.artikelNr, a.artikelBezeichnung, b.angebotsPreis
FROM bestellung b
         JOIN artikel a ON a.artikelNr = b.artikelNr
         JOIN warengruppe w ON w.gruppenNr = a.gruppenNr
WHERE w.gruppenName = 'DV-Papier'
