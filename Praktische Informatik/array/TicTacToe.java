...
public class TicTacToe {
    ...
    public void setzeSpielstein() {
        System.out.print("Eingabe Koordinate x (0-" + (spielfeld.length - 1) + "): ");
        int x = scanner.nextInt();
        System.out.print("Eingabe Koordinate y (0-" + (spielfeld.length - 1) + "): ");
    ...
    public boolean überprüfeDiagonaleLinksRechts() {
        boolean gleicheDiagonaleLinksRechts = true;
        for (int i = 0; i < spielfeld.length - 1; i++) {
            for (int j = i; j < spielfeld[i].length - 1; j++) {
                char aktuellesFeld = spielfeld[i][j];
                char nächstesFeld = spielfeld[i + 1][j + 1];
                if (aktuellesFeld != nächstesFeld || aktuellesFeld == '*') {
    ...
    public boolean überprüfeDiagonaleRechtsLinks() {
        boolean gleicheDiagonaleRechtsLinks = true;
        for (int i = 0; i < spielfeld.length - 1; i++) {
            for (int j = spielfeld[i].length - 1 - i; j > 0; j--) {
                char aktuellesFeld = spielfeld[i][j];
                char nächstesFeld = spielfeld[i + 1][j - 1];
                if (aktuellesFeld != nächstesFeld || aktuellesFeld == '*') {
    ...
    public void auswertungSpielfeld() {
        if (überprüfeZeilen() || überprüfeSpalten() || überprüfeDiagonaleLinksRechts()
